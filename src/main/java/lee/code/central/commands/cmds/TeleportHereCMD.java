...
public class TeleportHereCMD extends CustomCommand {
  private final Central central;

  public TeleportHereCMD(Central central) {
  ...
  public void perform(Player player, String[] args, Command command) {
    ...
    target.teleportAsync(player.getLocation()).thenAccept(result -> {
      if (result) player.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_TELEPORT_HERE_SUCCESSFUL.getComponent(new String[] {ColorAPI.getNameColor(target.getUniqueId(), target.getName())})));
      else player.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_TELEPORT_HERE_FAILED.getComponent(new String[] {ColorAPI.getNameColor(target.getUniqueId(), target.getName())})));
    });
  }
