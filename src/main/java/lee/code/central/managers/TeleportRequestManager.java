    ...
    Bukkit.getServer().getAsyncScheduler().runDelayed(central, scheduledTask -> {
      if (hasActiveRequest(playerID, targetID)) {
        ...
        if (oPlayer.isOnline()) {
          ...
          if (player != null)
            player.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.ERROR_TELEPORT_ASK_REQUEST_TIMEOUT_PLAYER.getComponent(new String[]{ColorAPI.getNameColor(targetID, oTarget.getName())})));
        }
        if (oTarget.isOnline()) {
          final Player target = oTarget.getPlayer();
          if (target != null)
            target.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.ERROR_TELEPORT_ASK_REQUEST_TIMEOUT_TARGET.getComponent(new String[]{ColorAPI.getNameColor(playerID, oPlayer.getName())})));
        }
